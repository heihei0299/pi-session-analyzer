...
- **Refresh/Watch 语义**：Refresh（`internal/refresh` + Pi/Codex adapter）为唯一写入路径，串行化，失败保快照并经 meta 暴露；Watch 先用 source-specific cheap revision，只有对应源变化才做该源 Refresh，失败保留 acknowledged revision 并重试；无独立 token/cost 聚合。`source=all` 未配置可用 Pi root 时采用明确 Codex-only 语义：不建立 Pi binding、不读取 Pi history；该降级仅适用于 ledger 确认没有 token-analyzer 自有 Pi history 的情况，发现无 binding 的 legacy Pi history 时 Refresh/Query 以 `ErrSourceRootBindingRequired` fail closed 且不自动认领；`source=pi` 仍拒绝不可用 root。root identity 校验通过后只使用同一个已固定的 canonical physical root，候选 project/`.jsonl` symlink 拒绝且不跟随，失败即拒绝；该保护为校验式 containment（枚举与读取/rename 前复核），不是 dirfd/renameat 级原子隔离，最后一次复核与系统调用之间仍有窗口。
