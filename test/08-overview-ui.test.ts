...
test("S13 总览骨架标记：8 卡片 / 分组切换 / 时间预设 / 状态行 / 导出按钮 / 错误横幅 / 深色主题", async () => {
  ...
  try {
    ...
    assert.match(body, /口径说明：统计含 pi 四载体/, "口径说明应声明四载体口径");
    assert.match(body, /assistant\/toolResult\/compaction\/branch_summary/, "口径说明应列出四载体类型");
    assert.match(body, /总输入/, "口径说明应声明总输入口径");
    assert.match(body, /其他客户端请求不计入|opencode 等其他客户端/, "口径说明应声明其他客户端不计入");
