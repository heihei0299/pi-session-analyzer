...
test("S7 缓存率聚合：先求和分子分母再除；分母为 0 的请求记 0 不报错", async () => {
  ...
  try {
    ...
    // 聚合缓存率 = ΣcacheRead / Σ(input+cacheRead+cacheWrite)
    //   = (200+100) / (100+400+200+100+10+0) = 300/810 = 37.04%
    //   （逐请求平均会得 (64.52%+20%)/2 = 42.26%，可区分）
    assert.equal(row["缓存率"], "37.04%", "聚合缓存率应为 300/810 = 37.04%");

    // 请求数 = 3（含全 0 失败消息）
    assert.equal(row["请求数"], "3", "请求数应为 3");
    // 总 token = 输入500 + 输出100 + 缓存读300 + 缓存写10 = 910
    assert.equal(row["总 token"], "910", "总 token 应为组件和 910");
    // 输入 = 500
