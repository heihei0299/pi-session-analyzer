...
- **数据源**：`--source pi|codex|all`（默认 `pi`）；Codex 目录优先级为 `--codex-dir > CODEX_HOME > ~/.codex`。`--dir` 始终只表示 Pi 目录。`all` 仅在 ledger 确认没有任何 token-analyzer 自有 Pi history、且未配置 Pi root 时按 Codex-only 查询：不创建 Pi binding、不读取 Pi history，Pi 部分为空；若发现无 binding 的 legacy Pi history，则 Refresh/Query 都以 `source root binding required` fail closed 且不改动历史行；`pi` source 仍会拒绝不可用 root。
- **数据库**：`--db <path>` 可指定 normalized ledger；未指定时使用 `TOKEN_ANALYZER_DB`/默认缓存路径。单个 ledger 只绑定一个 Pi root：首次 Pi Refresh 建立真实路径 binding，切换 `--dir` 后 Refresh/Query/QueryDetail 会明确拒绝且不删除历史行；旧 v2/无 binding ledger 的可写 migration 只补齐 schema；若已有 Pi 历史行，首次 Refresh 仍需显式迁移/确认或使用新 ledger，不能自动认领，只有空 ledger 才能首次绑定；缺失 root 或 symlink target 无法解析时安全拒绝；校验通过后 Refresh/Query/rename 只使用同一个 canonical physical root，候选 project/.jsonl symlink 一律拒绝且不跟随；失败即拒绝，游标、绑定与外部文件不变。这是校验式 containment（枚举与每次读取/rename 前复核 parent 与文件物理路径），不是 dirfd/renameat 级原子隔离，最后一次校验与实际系统调用之间仍有窗口。
