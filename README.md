...
领域术语见 [`CONTEXT.md`](CONTEXT.md)，关键决策见 [`docs/adr/`](docs/adr/)（当前：`0001-fork-session-dedup.md`）。
## 安装
...
- **时间范围**：今天 / 7天 / 30天 / 全部 / 自定义（date 日期 + 时分下拉，按本地时间解释），作用于总览与明细与导出；状态行「范围」随筛选即时显示（未筛选时显示数据范围 min/max）
...
- **fork 会话去重**：header 含 `parentSession` 的 fork 会话，其复制历史（`message.timestamp < header.timestamp`，fork 创建时间）的 usage 已在父会话统计过，analyzeFile 剔除；fork 后新增消息保留（CLI/webui/--watch 一致，watch 首读解析 forkTs 剔除、替换/重读复用；与 pi-switch 网关统计对齐，差异 ≈0.2%）
...
docs/adr/       架构决策记录（当前：0001-fork-session-dedup）
src/
