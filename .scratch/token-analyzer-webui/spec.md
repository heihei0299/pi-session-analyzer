...
| `GET /api/sessions` | model/cwd/since/until/page/size/sortKey/sortDir | `{ window, rows: SessionRow[], total }`，每行含 fileName（ticket 06 补充）；分页时含 page/size |
| `GET /api/requests` | model/cwd/since/until/page/size/sortKey/sortDir | `{ window, rows: RequestRow[], total }`；分页时含 page/size |
...
- 筛选参数 `model` / `cwd` / `since` / `until` 复用既有 `filterFiles`（since/until 闭区间、cwd 规范化比较）；时间过滤粒度（ticket 22/23 用户决策）：**统计端点**（totals/groups/period/requests）按**消息 timestamp** 消息级归属（跨天会话中落在范围内的请求/消耗计入当天，总览与请求明细数字一致）；**sessions 明细端点**按会话 header 归属（会话级窗口）；**CLI** 仍按会话 header（口径 A 不变）——webui 统计与 CLI 在跨天场景存在预期差异（spec 记录）
- 明细端点分页/排序（ticket 21）：`page`（≥1 整数）与 `size`（1-200 整数）须成对出现；`sortKey`（明细列集，`cache` 为 cacheRead+cacheWrite 别名）与 `sortDir`（asc/desc）须成对出现；响应恒含 `total`（筛选后全量行数），传了分页参数时 rows 为当前页并附 page/size；参数单独出现或非法 → 400
- 错误处理：统一 JSON 错误体 `{ error, detail }` + HTTP 状态码（400 参数非法 / 404 未知路径或会话不存在 / 409 活跃或重名 / 500 数据目录不可读或无合法会话）
- 服务端每次请求全量 `readSessionFiles(dir)` 后按参数过滤/分组/汇总（212 会话秒级处理，无常驻状态）；明细端点派生后**先排序再切页**（避免全量响应传输：真实数据 /api/requests 26.7MB → 分页后 ~20KB）

### 前端 UI 布局（ticket 03）

单页 SPA，四个 tab：**总览 / 会话明细 / 请求明细 / 会话管理**。

- **总览**：8 张汇总指标卡片（请求数 / 输入 / 输出(含推理) / 缓存(cacheRead+cacheWrite) / 推理 / 缓存率 / 总 token / 花费）；**输入 = 总输入（非缓存 input + 缓存命中 cacheRead）**，与 pi-switch 网关 Input 字段对齐（ticket 24；CLI input 仍为非缓存，导出 JSON/CSV 保持原始字段）；数值格式化 ≥10⁴ 用 k/M 缩写；花费全 0 显示「费率未配置（免费/未定价）」；分组表按模型/cwd 切换（列：分组键/请求数/输入/输出/缓存率/总token/花费）；「导出 JSON / 导出 CSV」按钮（前端 fetch 组装下载，不引入服务端下载端点）
- **会话明细**：列 = 会话名称/会话ID/时间/cwd/模型/请求数/输入/输出/缓存率/总token/花费（会话名称 = displayName：重命名过的用文件名前缀，否则首条 user 消息，截断+悬浮全文，可排序）
- **请求明细**：列 = 会话名称/会话ID/时间/模型/输入/输出/缓存/推理/缓存率/总token/花费（会话名称同会话明细，来自 /api/requests 行扩展字段 displayName）；默认按时间倒序；输入列 = 总输入（含缓存，ticket 24，缓存 ⊆ 输入）
- 明细表分页下拉（20/50/100），点击列头排序（ticket 21 起为**服务端分页**：翻页/排序/改页大小重新 fetch 带 page/size/sortKey/sortDir，响应 rows 为当前页 + total；不再前端内存全量拉取）；明细**应用时间范围筛选**（ticket 22：fetchRows/snapshot 携带 since/until，随「今天/7天/30天/自定义」预设）
...
- **视觉**：深色主题（仿参考页 zinc 暗色系），顶部标题 + 数据目录/会话数/范围状态行（数据源 `/api/meta`）；「范围」在存在时间筛选时显示**当前筛选范围**（纯日期 since 补 00:00 / until 补 23:59:59），未筛选（全部）时显示数据范围 min/max（ticket 22）
- 布局示意（原型）：
...
- **API 实现**：每次请求全量 `readSessionFiles(dir)` → 过滤 → 按端点派生（totalsFromFiles / sessionRowsFromFiles / requestRowsFromFiles / groupRowsFromFiles / periodRowsFromFiles）；响应字段复用 `serialize.ts` 的 `*ToObject` 转换；`/api/sessions` 行扩展 `fileName` 字段；明细端点（sessions/requests）派生后经 `applySort`（数值列数字比较、字符串列 localeCompare、cache 别名 = cacheRead+cacheWrite）→ `applyPaging`（slice）→ 响应含 `total`（ticket 21）；**过滤**：sessions 走 `loadFiltered`（filterFiles 全参数，会话级，CLI 口径）；totals/groups/period/requests 走 `loadFilteredMessageTime`（filterFiles 仅 model/cwd + `applyMessageTimeFilter` 按消息 timestamp，ticket 22/23）
...
- **统计口径**：沿用 token-analyzer 口径 A（analyze 层 filterFiles 未改）；新增 **fork 去重**（ticket 25，analyzeFile 数据层，CLI/webui 一致）：fork 会话（header.parentSession）中 message.timestamp < header.timestamp 的复制历史剔除（其 usage 已在原会话统计），fork 后新增消息保留；webui 在**端点层**将时间筛选改为消息级归属（ticket 22/23：totals/groups/period/requests 按消息 timestamp，sessions 按 header），CLI 保持会话级——跨天场景 webui 统计与 CLI 数字有预期差异，属用户决策（spec 记录）
