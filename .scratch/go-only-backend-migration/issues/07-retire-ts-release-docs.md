...
**What to build:** 当 Pi/Codex 全部生产能力已由 Go 覆盖后，删除 TypeScript/Node 后端与旧 oracle，统一 module/repository/release/README/CONTEXT/ADR，使用户只看到一个边界清晰、不含 OpenCode 的 Go-only token-analyzer。
...
- [ ] CONTEXT/ADR 更新为 ledger 唯一事实中心、Go-only、Refresh/Query 分离、Watch 新语义，以及 OpenCode 已解耦。
- [ ] 最终删除测试成立：无 TS backend、无旧 SessionData 生产 aggregate、无 Watch 独立统计、无 WebUI 双副本、无 OpenCode runtime/API/UI 耦合。
