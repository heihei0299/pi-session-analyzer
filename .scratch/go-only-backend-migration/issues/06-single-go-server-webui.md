...
**What to build:** 用户启动一个 Go binary 即可使用 Pi、Codex、All 的完整受支持 WebUI；UI 只有一个人工维护源，全部 API 由 Go 提供，没有 Node server、OpenCode 页面或双副本同步。

**Blocked by:** 02: 从 token-analyzer 移除 OpenCode 耦合; 03: Pi 全窗口切到 Go ledger + Query Engine; 04: Codex / All 切到同一个 ledger-native Query Engine; 05: 收口 Refresh / Query / Watch 生命周期.
...
- [ ] WebUI 与 HTTP API 不包含 OpenCode 专用交互或路由。
