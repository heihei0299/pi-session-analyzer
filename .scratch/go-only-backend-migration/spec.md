...
**实现目标**: 将生产后端完整收敛为 Go；Pi 与 Codex 统一写入 normalized SQLite ledger；所有统计窗口通过单一 Go Query Engine 查询；TypeScript 仅在迁移期作为行为 oracle，最终退出生产运行时；OpenCode 与当前项目彻底解耦。
...
- 当前仓库还包含 OpenCode RPC、同步、存储、HTTP API、WebUI audit 等独立领域能力，使 token-analyzer 同时承担“本地 Pi/Codex usage 分析”和“OpenCode 云端对账客户端”两个不同产品职责。
...
- **OpenCode 不属于 token-analyzer 的功能域**：当前仓库中的 OpenCode RPC、凭据、同步、storage、CLI/API、WebUI audit 集成全部解除，不迁移成 Go-only 核心能力，也不进入 normalized ledger、source union 或发布产物；
- 如果未来仍需要 OpenCode 对账，应在独立项目/插件/外部工具中实现，通过清晰边界与 token-analyzer 交互，而不是继续内嵌。

主测试 seam：

**source refresh → normalized ledger → Query Engine → CLI/API/WebUI 可观察结果**

OpenCode 解耦则以“token-analyzer 在没有任何 OpenCode 配置、凭据、网络接口和 UI 代码时仍完整工作”为验收 seam。
...
42. 作为项目所有者，我想让 OpenCode RPC/Seroval/cookie/workspace/sync/storage/audit 不再属于 token-analyzer，这样核心项目只维护 Pi/Codex usage 域。
43. 作为项目所有者，我想让 token-analyzer 的 HTTP API 不再暴露 OpenCode 专用端点。
44. 作为 WebUI 用户，我想让 token-analyzer UI 不再包含 OpenCode audit 页面或同步按钮。
45. 作为 CLI 用户，我想让 token-analyzer 不再包含 OpenCode 专用同步/导出命令。
46. 作为安全责任人，我想让 token-analyzer 不再处理 OpenCode 登录 cookie/凭据，这样项目减少一类敏感数据责任。
47. 作为维护者，我想让 OpenCode 本地 storage/cursor/lock 从本项目删除，这样存储模型只围绕 Pi/Codex。
48. 作为维护者，我想让未来 OpenCode 对账如有需要，以独立工具/插件实现，而不是重新耦合进核心。
...
- **OpenCode 作为独立领域从当前项目移除，不做 Go parity。**
- OpenCode 的 RPC client、Seroval、credential/cookie、workspace discovery、sync、storage、cursor、lock、export、HTTP API 与 WebUI audit 都不属于迁移后的 token-analyzer。
- token-analyzer 不保存、不读取、不要求任何 OpenCode 凭据。
- token-analyzer 不提供 source=opencode，也不把 OpenCode 加入 All。
- 如果未来需要 OpenCode 能力，应建立独立 repo/package/plugin，并通过公开稳定边界集成；该工作不属于本 spec。
...
- token-analyzer 在没有 OpenCode 配置、cookie、网络访问和本地 OpenCode data 的环境中完整运行。
- CLI help/API routes/WebUI 不再暴露 OpenCode 专用能力。
- 发布产物与运行依赖中不存在 OpenCode 专用 runtime/config requirement。
- 删除 OpenCode 代码后 Pi/Codex canonical contract 完全不变。
...
- 为 OpenCode 功能寻找新宿主、设计新插件或创建新项目。
...
- 本 spec 的完成标准不是“没有 .ts”，而是 usage 统计领域只剩一个生产实现，并且 OpenCode 不再是本项目职责。
- OpenCode 的解除耦合是产品边界收缩，不是“把 OpenCode 迁到 Go”。
